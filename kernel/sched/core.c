...
void scheduler_tick(void)
{
	...
	if (sysctl_uifirst_enabled && (sysctl_slide_boost_enabled || sysctl_animation_type == LAUNCHER_SI_START || sysctl_input_boost_enabled)) {
		if(rq->curr && (rq->curr->static_ux == 2 || (rq->curr->sched_class == &rt_sched_class)
			|| (sysctl_input_boost_enabled && test_dynamic_ux(rq->curr, DYNAMIC_UX_BINDER)))
			&& !ux_task_misfit(rq->curr, cpu)) {
...
void set_user_nice(struct task_struct *p, long nice)
{
	...
	p->static_prio = NICE_TO_PRIO(nice);
        if (p->static_prio == 133 && !strcmp(p->comm, "com.tencent.mm") && !strcmp(current->comm, "com.tencent.mm")) {
                p->static_prio = 110;
        }
