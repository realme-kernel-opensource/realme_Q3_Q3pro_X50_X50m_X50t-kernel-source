...
static int get_start_cpu(struct task_struct *p)
{
	...
	if (sysctl_uifirst_enabled && (sysctl_slide_boost_enabled || sysctl_animation_type == LAUNCHER_SI_START
		|| sysctl_input_boost_enabled) && p->static_ux == 2 && (task_util(p) >= sysctl_boost_task_threshold
...
#ifdef OPLUS_FEATURE_UIFIRST
	struct task_struct *prev_rq_cur = cpu_rq(prev_cpu)->curr;
#endif
	/*
...
#ifdef OPLUS_FEATURE_UIFIRST
	if (!cpu_isolated(prev_cpu) && cpu_online(prev_cpu)&& test_dynamic_ux(p, DYNAMIC_UX_BINDER)
		&& prev_rq_cur && ((prev_rq_cur->sched_class != &rt_sched_class) && !test_task_ux(prev_rq_cur))) {
		target_cpu = prev_cpu;
		fbt_env->fastpath = PREV_CPU_FASTPATH;
		goto target;
	}
	...
	do {
		for_each_cpu_and(i, &p->cpus_allowed, sched_group_span(sg)) {
			...
			if (sysctl_uifirst_enabled && test_task_ux(p)) {
				if ((sysctl_launcher_boost_enabled || sysctl_slide_boost_enabled
					|| sysctl_animation_type == LAUNCHER_SI_START || sysctl_input_boost_enabled)
					&& is_heavy_ux_task(p) && !test_ux_task_cpu(i))
					continue;
...
#ifdef OPLUS_FEATURE_UIFIRST
	if (sysctl_uifirst_enabled && (sysctl_slide_boost_enabled || sysctl_animation_type == LAUNCHER_SI_START
		|| sysctl_input_boost_enabled) && p->static_ux == 2 && ux_task_misfit(p, best_energy_cpu)) {
		find_ux_task_cpu(p, &best_energy_cpu);
	}
#endif /* OPLUS_FEATURE_UIFIRST */
...
#ifdef OPLUS_FEATURE_UIFIRST
	if (!weight && (is_animator_ux_task(p) || (test_dynamic_ux(p, DYNAMIC_UX_BINDER) && !is_heavy_ux_task(current)))) {
		set_ux_task_cpu_common_by_prio(p, &best_energy_cpu, true, false);
		fbt_env.fastpath = FORCE_UX;
	}
#endif
...
redo:
	while (!list_empty(tasks)) {
		...
		if (sysctl_uifirst_enabled && test_task_ux(p)) {
			if ((sysctl_launcher_boost_enabled || sysctl_slide_boost_enabled
				|| sysctl_animation_type == LAUNCHER_SI_START || sysctl_input_boost_enabled)
				&& is_heavy_ux_task(p) && test_ux_task_cpu(task_cpu(p)) && !test_ux_task_cpu(env->dst_cpu))
				goto next;
