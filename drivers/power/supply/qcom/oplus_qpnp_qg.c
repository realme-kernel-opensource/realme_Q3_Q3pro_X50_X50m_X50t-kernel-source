...
/* ZhangKun@BSP.CHG.Basic 2020/11/09, Add for battery proc devinfo */
...
#ifdef OPLUS_FEATURE_CHG_BASIC
/* Zengpeng.Chen@BSP.CHG.Basic 2020/12/28 add for Batt_NTC ADC compensation */
...
#ifdef OPLUS_FEATURE_CHG_BASIC
/* ZhangKun@BSP.CHG.Basic 2020/11/09, Add for batt ID check*/
...
#ifdef OPLUS_FEATURE_CHG_BASIC
//qg-util.c start, ZhangKun@BSP.CHG.Basic 2020/11/09, Add for oplus_gauge
...
/* ZhangKun@BSP.CHG.Basic 2020/11/09, While batt_NTC & batt_ID both invalid return -40C */
	if (chip->battery_missing) {
		*temp = 250;
		return 0;
	}
...
#ifdef OPLUS_FEATURE_CHG_BASIC
/* Zengpeng.Chen@BSP.CHG.Basic 2020/12/28 modify for Batt_NTC ADC compensation */
	if (chip->adc_compensation_enabled) {
		...
		pr_debug("batt_temp after Batt_NTC ADC compensation = %d\n", *temp);
	} else {
...
#endif
//qg-util.c end, ZhangKun@BSP.CHG.Basic 2020/11/09, Add for oplus_gauge
...
#ifndef OPLUS_FEATURE_CHG_BASIC
/* ZhangKun@BSP.CHG.Basic 2020/11/09, Modify Fake battery ID range */
...
static void qg_notify_charger(struct qpnp_qg *chip)
{
	...
	if (!chip->profile_loaded)
		return;
#ifdef OPLUS_FEATURE_CHG_BASIC
	/* ZhangKun@BSP.CHG.Basic 2020/11/09, Fcv only can be setted by opluschg_set_fv,but no battery profile max-voltage-uv */
#else
...
{
#ifdef OPLUS_FEATURE_CHG_BASIC
	/* wangxianfei@BSP.CHG.Basic 2020/08/20,  Ignore ESR if battery-temp is below a threshold */
...
#ifdef OPLUS_FEATURE_CHG_BASIC
/* ZhangKun@BSP.CHG.Basic 2020/11/09, Add to fix BUG 2745009 */
			       ...
			       union power_supply_propval *pval)
{
	...
	case POWER_SUPPLY_PROP_RESISTANCE_ID:
		/* ZhangKun@BSP.CHG.Basic 2020/11/09 Fix battery id accuracy loss */
		#ifdef OPLUS_FEATURE_CHG_BASIC
...
static int qg_battery_status_update(struct qpnp_qg *chip)
{
	...
	pr_err("qg_battery_status_update battery_missing = %d prop.intval = %d \n",chip->battery_missing, prop.intval);
...
#ifdef OPLUS_FEATURE_CHG_BASIC
/* ZhangKun@BSP.CHG.Basic 2020/11/09 Add battery id check */
...
static int qg_load_battery_profile(struct qpnp_qg *chip)
{
	...
	if (chip->dt.multi_profile_load) {
		...
		#ifdef OPLUS_FEATURE_CHG_BASIC
		/* ZhangKun@BSP.CHG.Basic 2020/11/09 Fix loss of accuracy when divide */
...
static int qg_setup_battery(struct qpnp_qg *chip)
{
	...
	if (!is_battery_present(chip)) {
		...
		if (rc < 0) {
			...
			/* ZhangKun@BSP.CHG.Basic 2020/11/09 Calculate battery id by rounding */
#ifdef OPLUS_FEATURE_CHG_BASIC
...
#ifndef OPLUS_FEATURE_CHG_BASIC
	/* ZhangKun@BSP.CHG.Basic 2020/11/09, Add for oplus gauge*/
...
#ifdef OPLUS_FEATURE_CHG_BASIC
/* ZhangKun@BSP.CHG.Basic 2020/11/09, Add for oplus_gauge*/
...
{
	int batt_fcc = 5000;//full charge capacity
	return batt_fcc;
}
...
static int oplus_get_battery_remain_capacity(void)
{
	...
	int rc = 0;
	rc = qg_get_battery_capacity(qpnp_gauge_ic, &remain_capacity);
	remain_capacity = 5000 * remain_capacity / 100;
	pr_debug("oplus_get_battery_remain_capacity = %d, rc= %d\n",remain_capacity,rc);
...
#ifdef OPLUS_FEATURE_CHG_BASIC
    /* ZhangKun@BSP.CHG.Basic 2020/11/09  add parallel current now */
