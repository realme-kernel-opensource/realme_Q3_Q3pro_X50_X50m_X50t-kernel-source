...
static int bq2589x_inform_charger_type(struct bq2589x *bq)
{
	...
	//ret = power_supply_get_property(bq->psy,
...
int oplus_bq2589x_set_ichg(int cur)
{
	...
	if (g_bq->pre_cur == cur) {
		...
		g_bq->pre_cur = cur;
		pr_notice("%s: g->bq->pre_cur = %d, cur = %d\n",
				__func__, g_bq->pre_cur, cur);
		if(cur <= 1000){   // <= 1A
...
int oplus_bq2589x_set_qc_config(void)
{
	...
	if (chip->limits.vbatt_pdqc_to_5v_thr > 0 && chip->charger_volt > 7500
		...
		dev_err(g_bq->dev, "%s:qc Force output 9V\n",__func__);
