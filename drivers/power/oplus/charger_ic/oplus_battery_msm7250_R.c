...
void smblib_usb_plugin_locked(struct smb_charger *chg)
{
	...
	if (vbus_rising) {
		...
		if (smblib_get_prop_dfp_mode(chg) != POWER_SUPPLY_TYPEC_NONE) {
			...
			oplus_wake_up_usbtemp_thread();
			printk(KERN_ERR "!!!!! smblib_usb_plugin_locked: [%d]  POWER_SUPPLY_TYPEC_NONE\n", vbus_rising);
			return;
		}
#endif
...
int oplus_chg_set_pd_config(void)
{
	...
	} else if (chip->vbatt_num == 1 && is_vooc_support_single_batt_svooc() == false) {
...
/* Yichun.Chen  PSW.BSP.CHG  2019-08-19  for c to c */
	...
	case POWER_SUPPLY_PROP_FAST_CHG_TYPE:
		if(((oplus_vooc_get_fastchg_started() == true) ||
			...
			(g_oplus_chip->vbatt_num == 1)&&(oplus_vooc_get_fast_chg_type() == FASTCHG_CHARGER_TYPE_UNKOWN)&&(g_oplus_chip->vooc_project == 1)){
