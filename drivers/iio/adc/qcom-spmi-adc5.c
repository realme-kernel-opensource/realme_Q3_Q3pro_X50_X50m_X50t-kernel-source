...
/* Zengpeng.Chen@BSP.CHG.Basic 2020/12/28 add for Batt_NTC ADC compensation */
	case IIO_CHAN_INFO_OFFSET:
		...
		if (*val2 < 0) {
			...
			pr_debug("%s : adc_code_volt before compensation : %d(uV)\n", __func__, voltage);
			voltage += *val2;
			pr_debug("%s : adc_code_volt after compensation : %d(uV)\n", __func__, voltage);
...
/*ZhangKun@BSP.CHG.MISC, modify for the hw difference of adc 2020/12/03*/
