...
static void parse_light_sensor_dts(struct sensor_hw* hw, struct device_node *ch_node)
{
    ...
    for (di = 0; di < ARRAY_SIZE(light_para); di++) {
        ...
        SENSOR_DEVINFO_DEBUG("light_para[%s] : %d\n", als_feature[di], hw->feature.parameter[di]);
    }
}
...
static void parse_gsensor_sensor_dts(struct sensor_hw* hw, struct device_node *ch_node) {
    int rc = 0;
    int value = 0;
    rc = of_property_read_u32(ch_node, "gyro_factor", &value);

    if (!rc) {
        hw->feature.feature[0] = value;
    } else {
        hw->feature.feature[0] = 10000;//set default
    }

    SENSOR_DEVINFO_DEBUG("parse_gsensor_sensor_dts gyro_factor: %d\n", hw->feature.feature[0]);
}
...
static void parse_each_physical_sensor_dts(struct sensor_hw* hw, struct device_node *ch_node)
{
    if (0 == strncmp(ch_node->name, "msensor", 7)) {
        ...
        parse_gsensor_sensor_dts(hw, ch_node);
    } else {
        //do nothing
    }
