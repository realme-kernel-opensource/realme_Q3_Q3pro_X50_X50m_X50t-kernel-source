...
static bool oplus_sleep_disabled;
void set_lpm_disallowed_flag(bool disable)
{
	pr_err("set_lpm_disallowed_flag: old flag[%d], new flag[%d]\n",sleep_disabled, disable);
	oplus_sleep_disabled = disable;
	return;
}
...
static inline bool lpm_disallowed(s64 sleep_us, int cpu, struct lpm_cpu *pm_cpu)
{
	...
	if (cpu_isolated(cpu))
		goto out;
/* yanghao@BSP.Kernel.Stability when booting will meet system change the sleep
	 * will caused insert usb but sleep disabled = false 2020-4-7
	 */
#ifdef OPLUS_FEATURE_CHG_BASIC
	if (sleep_disabled || oplus_sleep_disabled)
#else
	if (sleep_disabled)
#endif
		return true;
