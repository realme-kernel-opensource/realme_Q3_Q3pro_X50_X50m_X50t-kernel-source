...
static int nvt_set_jitter_para(void *chip_data, int level)
{
    int8_t ret = -1;
    struct chip_data_nt36672c *chip_info = (struct chip_data_nt36672c *)chip_data;

    TPD_INFO("%s:jitter value = %d, chip_info->is_sleep_writed = %d\n", __func__, level, chip_info->is_sleep_writed);
    ret = nvt_extend_cmd2_store(chip_info, EVENTBUFFER_EXT_CMD, EVENTBUFFER_EXT_JITTER_LEVEL, level);
    return ret;
}

static int nvt_set_smooth_para(void *chip_data, int level)
{
    int8_t ret = -1;
    struct chip_data_nt36672c *chip_info = (struct chip_data_nt36672c *)chip_data;
...
//    .get_usb_state              = nvt_get_usb_state,
    ...
    .smooth_lv_set              = nvt_set_smooth_para,
    .sensitive_lv_set           = nvt_set_jitter_para,
};
