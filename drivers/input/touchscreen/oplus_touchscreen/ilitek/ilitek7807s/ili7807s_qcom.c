...
#endif
    ilits->ws = wakeup_source_register(ilits->ts->dev, "ili_wakelock");
...
static int ilitek_cs_gpio_control(void *chip_data, bool enable)
{
    struct ilitek_ts_data *chip_info = (struct ilitek_ts_data *)chip_data;
    if (gpio_is_valid(chip_info->hw_res->cs_gpio)) {
        ILI_INFO("%s: set cs state %d\n", __func__, enable);
...
static struct oplus_touchpanel_operations ilitek_ops = {
    ...
    .cs_gpio_control            = ilitek_cs_gpio_control,
...
abnormal_register_driver:
    ilitek_cs_gpio_control(ilits, false);
    ilits->ws = wakeup_source_register(ts->dev, "ili_wakelock");
