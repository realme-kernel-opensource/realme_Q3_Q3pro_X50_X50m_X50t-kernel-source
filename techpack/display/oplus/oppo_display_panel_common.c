...
int oppo_display_panel_get_serial_number(void *buf)
{
	...
	ret = utils->read_u32(utils->data, "qcom,mdss-panel-serial-number-index", &val);
	if (ret) {
	...
	for (i = 0;i < 10; i++) {
		if (!strcmp(display->panel->name, "samsung ams643ye01 amoled fhd+ panel")) {
			...
			ret = dsi_display_read_panel_reg(get_main_display(), 0xD8, read, 17);
		} else {
...
int oplus_display_get_panel_reg(void *data)
{
	...
	u32_bytes = oppo_rx_len%u32_bytes ? (oppo_rx_len/u32_bytes + 1) : oppo_rx_len/u32_bytes;
	oplus_big_endian_copy(panel_reg->reg_rw, oppo_rx_reg, u32_bytes);
	panel_reg->lens = oppo_rx_len;
