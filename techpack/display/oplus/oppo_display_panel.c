...
static const struct panel_ioctl_desc panel_ioctls[] = {
	...
	PANEL_IOCTL_DEF(PANEL_IOCTL_SET_APOLLO_BACKLIGHT, oplus_display_set_apollo_backlight_maplist),
	PANEL_IOCTL_DEF(PANEL_IOCTL_SET_APOLLO_VBACKLIGHT, oplus_display_set_apollo_backlight_vmaplist),
...
long panel_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	pr_err("%s pid = %d, cmd = %s, size=%d\n", __func__, task_pid_nr(current), ioctl->name, ksize);
...
static int __init oppo_display_panel_init()
{
	...
	oplus_apollo_backlight_list_alloc();
	return 0;
