...
extern bool is_lcd_cabc_support;
		...
		const char *buf, size_t count)
{
	...
	if (get_oppo_display_power_status() == OPPO_DISPLAY_POWER_ON) {
		...
		if (0 == oppo_last_backlight && is_lcd_cabc_support) {
...
struct device_attribute *attr, char *buf) {
	...
	for (i = 0;i < 10; i++) {
		if (!strcmp(display->panel->name,"samsung amb655uv01 amoled fhd+ panel with DSC")) {
		   ...
		   !strcmp(display->panel->name, "samsung ams643ye01 in 20057 amoled fhd+ panel") ||
		   !strcmp(display->panel->name, "samsung ams643ye01 in 20127 amoled fhd+ panel")) {
			if (display->panel->panel_initialized) {
				mutex_lock(&display->display_lock);
				mutex_lock(&display->panel->panel_lock);
			...
			if (!strcmp(display->panel->oppo_priv.vendor_name, "S6E3HC3")) {
		...
		if (!strcmp(display->name, "qcom,mdss_dsi_oppo19125samsung_ams644va04_1080_2400_video")) {
			...
			*/
			...
			} else if (!strcmp(display->panel->oppo_priv.vendor_name, "S6E3HC3")) {
...
static ssize_t oppo_display_get_debug(struct device *dev,
                                struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d %d %d %d %d %d\n", oppo_dimlayer_bl_on_vblank, oppo_dimlayer_bl_off_vblank, oppo_fod_on_vblank, oppo_fod_off_vblank, oppo_datadimming_v3_debug_value, oppo_datadimming_v3_debug_delay);
}
...
void oppo_panel_process_dimming_v2_post(struct dsi_panel *panel, bool force_disable)
{
	...
	if (oppo_datadimming_v2_need_flush) {
		if (oppo_datadimming_v2_need_sync && strcmp(panel->oppo_priv.vendor_name, "AMB655XL08")
...
int oppo_panel_process_dimming_v2(struct dsi_panel *panel, int bl_lvl, bool force_disable)
{
	...
	if (!force_disable && oppo_dimlayer_bl_enable_v2_real &&
		...
		if (!oppo_seed_backlight) {
			...
			oppo_seed_backlight = 1;
			lcdinfo_notify(LCM_DC_MODE_TYPE, &oppo_seed_backlight);
			oppo_dc_v2_on = true;
	...
	if (!strcmp(panel->oppo_priv.vendor_name, "AMB655XL08")) {
		if (oppo_datadimming_v2_need_flush && oppo_dc_v2_on)
...
int dsi_update_dynamic_osc_clock(void)
{
	...
	if (osc_clock_rate) {
		if (osc_clock_rate == display->panel->oppo_priv.osc_clk_mode0_rate) {
			rc = dsi_panel_tx_cmd_set(display->panel, DSI_CMD_OSC_CLK_MODEO0);
		} else if (osc_clock_rate == display->panel->oppo_priv.osc_clk_mode1_rate) {
		...
		struct device_attribute *attr, char *buf)
{
	...
	if (!(display && display->panel->oppo_priv.vendor_name) ||
		...
		!strcmp(display->panel->oppo_priv.vendor_name, "AMB655XL08")) {
		ccd_check = 0;
		goto end;
	}
