...
/* Yuwei.Zhang@RM.MM.Display.LCD.Machine 2019.10.29 */
...
#define MIN_LCD_BACKLIGHT_LOWEST      13
...
__attribute__((weak)) int tp_control_reset_gpio(bool enable) {return 0;}
...
static int dsi_panel_power_on(struct dsi_panel *panel)
{
...
#ifdef CONFIG_REGULATOR_TPS65132
	if ((!panel->oppo_priv.is_tps65132_support) && (strstr(panel->oppo_priv.vendor_name,"NT36672C")==NULL)) {
#endif /*CONFIG_REGULATOR_TPS65132*/
		if ((0 == mdss_tp_black_gesture_status()) || (1 == tp_black_power_on_ff_flag) || (shutdown_flag == 1)) {
			...
			if (rc) {
		        ...
		        if (strstr(panel->oppo_priv.vendor_name,"ILI7807S") != NULL)
	...
	if (!strcmp(panel->name,"samsung amb655uv01 amoled fhd+ panel with DSC") ||
	    ...
	    !strcmp(panel->name,"samsung ams643ye01 in 20127 amoled fhd+ panel")) {
...
#ifdef CONFIG_REGULATOR_TPS65132
	if (panel->oppo_priv.is_tps65132_support) {
...
static int dsi_panel_power_off(struct dsi_panel *panel)
{
	...
	if(!esd_check && mdss_tp_black_gesture_status()) {
		tp_black_power_on_ff_flag = 0;
		if (shutdown_flag == 0) {
			pr_err("[TP] tp gesture is enable, not to dsi_panel_power_off, tp_black_power_on_ff_flag = %d\n",
					tp_black_power_on_ff_flag);
			return rc;
		}
	}
	tp_black_power_on_ff_flag = 1;
	pr_err("%s:[TP]tp_black_power_on_ff_flag = %d, shutdown_flag = %d\n",__func__,tp_black_power_on_ff_flag, shutdown_flag);
...
#ifdef CONFIG_REGULATOR_TPS65132
	/* Yuwei.Zhang@RM.MM.Display.LCD.Machine 2019.10.29 to control lcd power */
	if (panel->oppo_priv.is_tps65132_support) {
		TPS65132_pw_enable(0);
		usleep_range(100000, 100100);
	}
...
extern int oppo_display_mode;
static int (*dsi_panel_update_backlight_dcs)(struct dsi_panel *panel, u32 bl_lvl);
static int is_max_lite(void)
{
	return ((20620==get_project()) || (20621==get_project()) || (20622==get_project()) || (20690==get_project()));
}
static int is_zeus(void)
{
        return ((20669==get_project()) || (20750==get_project()) || (20751==get_project()));
}
struct _bl_map_rang {
	int start;
	int end;
...
static struct _bl_map bl_map[BL_MAP_NUM] = {
	{{1, 411},     {8, 175},     {406481, 1000000}},
	{{411, 820},   {175, 458},   {691883, 1000000}},
	{{820, 1229},  {458, 981},   {127999, 100000}},
	{{1229, 1638}, {981, 1887},  {221403, 100000}},
	{{1638, 2047}, {1887, 3340}, {355193, 100000}},
	{{2047, 4095}, {3340, 4095}, {369079, 1000000}},
};
static u32 clac_backlightness(struct dsi_panel *panel, u32 bl_lvl)
{
	u32 ret = 0;
	int i = 0;
	int find = -1;
	...
	if(0 != bl_lvl) {
		...
		if(i >= BL_MAP_NUM) {
			...
			ret = min(ret, (u32)bl_map[i].ic.end);
		}
	}

	return ret;
}
static void check_bl_min_time(void)
{
	int sleep_cnt = 6;
	int fd;
	do {
		fd = atomic_read(&bl_frame_done_cnt);
		if(0 == fd) {
			break;
		} else {
...
static int dsi_panel_update_backlight_vid_lcd(struct dsi_panel *panel,
		    u32 bl_lvl)
{
	static u32 bl_pre = U32_MAX;
	int rc = -1;
	...
	if((bl_pre != bl_lvl) || (bl_pre == U32_MAX)) {
		if(0 == bl_pre) {
			check_bl_min_time();
		}
		...
		if(dsi && dsi->host && dsi->host->ops && dsi->host->ops->transfer) {
			...
			rc = dsi->host->ops->transfer(dsi->host, &msg);
			if (rc < 0) {
				DSI_ERR("failed to send 0xff 0x10\n");
			}
			...
			rc = dsi->host->ops->transfer(dsi->host, &msg);
			if (rc < 0) {
				DSI_ERR("failed to send 0xfb 0x01\n");
			}

			bl = clac_backlightness(panel, bl_lvl);
			payload[0] = MIPI_DCS_SET_DISPLAY_BRIGHTNESS;
			payload[1] = bl >> 8;
			payload[2] = bl & 0xff;
			memset(&msg, 0, sizeof(msg));
			msg.channel = dsi->channel;
			...
			msg.type = MIPI_DSI_GENERIC_LONG_WRITE;
			msg.flags = MIPI_DSI_MSG_USE_LPM | MIPI_DSI_MSG_LASTCOMMAND;
			rc = dsi->host->ops->transfer(dsi->host, &msg);
			//	rc = mipi_dsi_dcs_set_display_brightness(dsi, bl);
			if (rc < 0) {
				DSI_ERR("failed to update dcs backlight:%d\n", bl);
			} else {
				bl_pre = bl_lvl;
				DSI_INFO("update dcs backlight: target = %d, actual = %d\n", bl_lvl, bl);
			}
		} else {
			DSI_ERR("%s: input param is invalid\n", __func__);
		}
	} else {
		rc = 0;
	}
...
#ifdef OPLUS_BUG_STABILITY
extern int oplus_panel_update_bl_hbm(u32 bl_lvl, struct dsi_panel *panel);
#endif /* OPLUS_BUG_STABILITY */
	...
	u32 bl_lvl)
{
	...
	if(OPPO_DISPLAY_AOD_SCENE != get_oppo_display_scene()){
		...
		if ((!strcmp(panel->name,"samsung ams643ye01 amoled fhd+ panel") ||
			!strcmp(panel->name,"samsung ams643ye01 in 20057 amoled fhd+ panel")) &&
			OPPO_DISPLAY_NORMAL_SCENE == get_oppo_display_scene()) {
...
#ifdef OPLUS_FEATURE_BACKLIGHT_GAMMA
	if (panel->oppo_priv.is_lcd_gamma_support) {
		if (bl_lvl <= MIN_LCD_BACKLIGHT_LOWEST && bl_lvl > 0) {
			backlight_gamma = 1;
			DSI_INFO("DSI_GAMMA_LOWBL_COMMAND, bl_lvl = %d\n", bl_lvl);
			rc = dsi_panel_tx_cmd_set(panel, DSI_GAMMA_LOWBL_COMMAND);
		} else if (bl_lvl > MIN_LCD_BACKLIGHT_LOWEST && backlight_gamma == 1){
			DSI_INFO("DSI_GAMMA_NOMAL_COMMAND, bl_lvl = %d\n", bl_lvl);
			backlight_gamma = 0;
...
#ifdef OPLUS_BUG_STABILITY
		if(dsi_panel_update_backlight_dcs) {
			rc = dsi_panel_update_backlight_dcs(panel, bl_lvl);
		}
		else {
			DSI_ERR("dsi_panel_update_backlight_dcs = NULL!\n");
		}
#else
...
#ifdef OPLUS_BUG_STABILITY
		if(is_max_lite())
			dsi_panel_update_backlight_dcs = dsi_panel_update_backlight_vid_lcd;
		else
			dsi_panel_update_backlight_dcs = dsi_panel_update_backlight;
#endif
		break;
	case DSI_BACKLIGHT_EXTERNAL:
		break;
...
#ifdef CONFIG_REGULATOR_TPS65132
/*Jianxiong.Zeng@PSW.MM.Display.LCD.Feature 2021.01.27 Modify for NT36672C IC power on sequence*/
	if ((panel->oppo_priv.is_tps65132_support)||(strstr(panel->oppo_priv.vendor_name,"NT36672C")!=NULL)) {
...
int dsi_panel_prepare(struct dsi_panel *panel)
{
	...
	#ifdef OPLUS_BUG_STABILITY
	if (is_zeus()) {
		if((get_boot_mode() != MSM_BOOT_MODE__FACTORY) && (get_boot_mode() != MSM_BOOT_MODE__RF) && (get_boot_mode() != MSM_BOOT_MODE__WLAN))
...
#ifdef OPLUS_BUG_STABILITY
	if (strstr(panel->oppo_priv.vendor_name,"NT36672C")!=NULL) {
#ifdef OPLUS_FEATURE_TP_BASIC
