...
static void cam_ife_mgr_print_io_bufs(struct cam_packet *packet,
	...
	bool *mem_found, struct cam_ife_hw_mgr_ctx *ctx)
#else
static void cam_ife_mgr_print_io_bufs(struct cam_packet *packet,
	int32_t iommu_hdl, int32_t sec_mmu_hdl, uint32_t pf_buf_info,
	bool *mem_found)
...
static int cam_ife_mgr_cmd(void *hw_mgr_priv, void *cmd_args)
{
	...
	case CAM_HW_MGR_CMD_DUMP_PF_INFO:
		cam_ife_mgr_print_io_bufs(
			...
			hw_cmd_args->u.pf_args.mem_found, ctx);
...
#ifdef OPLUS_FEATURE_CAMERA_COMMON
		CAM_WARN(CAM_ISP, "Add affected ctx %d to list",
			ife_hwr_mgr_ctx->ctx_index);
#else
...
#ifdef OPLUS_FEATURE_CAMERA_COMMON
		CAM_ERR(CAM_ISP, "Vfe core %d is affected (%d)",
...
#ifdef OPLUS_FEATURE_CAMERA_COMMON
		CAM_ERR(CAM_ISP, "CAM_ISP_HW_ERROR_CSID_FATAL %d enable_csid_recovery %d",
...
static int cam_ife_hw_mgr_debug_register(void)
{
	...
	CAM_INFO(CAM_ISP, "set enable_csid_recovery %d", g_ife_hw_mgr.debug_cfg.enable_csid_recovery);
#endif
